...
> 本 README 对应 **Beta 4.1 Windows 热修复版**与 **Beta 4 Apple Silicon Mac 版**。公开可见版本、下载资产与 SHA-256 始终以 Releases 页面实际列出的内容为准。
...
这个链接目前不代表已经与点力 Token 签署分成、返佣或代理条款。用户是否能在自身网络环境使用，取决于服务本身和实际连通性；控制台不替服务商保证兼容，也不宣传“无需 VPN”。
...
这两条 ASR 成功证据不能证明 LLM Provider 兼容，控制台也不据此认证任何中转站。Windows 云端 ASR 按 **Beta 4 实验功能**提供；macOS 云端语音当前不可用。语音合成 / TTS 继续等待服务商提供并验证接口、模型、音色、计费和数据处理资料。
...
- Windows 11 x64 与 Apple Silicon 包来自同一提交，已完成自动化、真实启动、升级保留和发布内容扫描。

Provider Hub 在 macOS 上必须先完成系统 Keychain 凭据实现与真机验证，不能把 Windows DPAPI 方案直接当成跨平台完成。

### Beta 4.1 Windows 热修复

- 修复启用自定义 Provider 后，Codex 本体对话范围被跟着切换的问题；
- 自定义 Provider 现在只作用于控制台自己的运行，不再改写 Codex 本体当前 Provider；
- “导入本机 Codex 对话”始终读取完整本机对话列表，不再被控制台当前模型来源过滤；
- Apple Silicon Mac 的自定义 Provider 原本就未启用，因此 Mac 继续使用已验收的 Beta 4 包。
...
只从本仓库的 [Releases](https://github.com/springwen131130-del/codex-biomechanical-console/releases) 下载，并按下面的 SHA-256 核对文件。Beta 4.1 Windows 资产由核心提交 `fb81c82` 构建；Apple Silicon Mac 继续使用核心提交 `ad60e58` 的 Beta 4 资产。两者都经过真实启动或对应平台验收与发布内容扫描。Windows 尚未商业代码签名，Mac 只有 ad-hoc 临时签名且尚未 Apple 公证。

| 平台 | 文件 | SHA-256 |
| --- | --- | --- |
| Windows 普通安装 | [`CODEX-Biomechanical-Console-Windows-Beta4.1-Setup-fb81c82.exe`](https://github.com/springwen131130-del/codex-biomechanical-console/releases/download/v0.1.0-beta.4.1/CODEX-Biomechanical-Console-Windows-Beta4.1-Setup-fb81c82.exe) | `745E4130B7F0312F0272F1E7CF4513253D20591EA459081AFEC875243518E645` |
| Windows 免安装 / 恢复 | [`CODEX-Biomechanical-Console-Windows-Beta4.1-fb81c82.zip`](https://github.com/springwen131130-del/codex-biomechanical-console/releases/download/v0.1.0-beta.4.1/CODEX-Biomechanical-Console-Windows-Beta4.1-fb81c82.zip) | `D9DF2575F081F7C49733C62332080E001C4D7DAE356D4D8ACC41A647C44F0259` |
...
| Windows 校验清单 | [`SHA256SUMS.txt`](https://github.com/springwen131130-del/codex-biomechanical-console/releases/download/v0.1.0-beta.4.1/SHA256SUMS.txt) | — |
